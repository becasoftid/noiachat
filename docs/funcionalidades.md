...
| Conversaciones | Operativo | Inbox redisenado, no leidos, filtros rapidos, vista tipo chat, panel de detalles y composer inteligente para atencion diaria. |
...
| CONV-002 | Conversaciones | Timeline entrante/saliente | Operativo | P0 | Vista tipo chat integrada en `/conversations?conversation=...` con lista lateral, cabecera de contacto/estado, burbujas, fechas, estados, errores, panel de detalles y compositor inferior segun ventana 24h | Agregar scroll automatico al ultimo mensaje si el volumen lo exige. |
...
| CONV-007 | Conversaciones | Redisenio UX/UI tipo WhatsApp | Operativo | P1 | `/conversations` queda organizado en inbox, conversacion activa y panel de detalles; el composer muestra texto/adjunto solo con ventana abierta y plantilla con campos individuales cuando esta cerrada; documentado en `docs/conversaciones-flujo-operativo.md`; validado con `NoiaChatMvpTest`, `php artisan test` y `npm run build` | Hacer validacion visual con operadores reales y capturas por breakpoint cuando se cierre QA. |
