...
| ONBOARD-001 | Onboarding | Registro con empresa, sede y plan basico de prueba | MVP | P1 | `/register` crea usuario `company_admin`, empresa, sede inicial, membresia activa y suscripcion `trialing` al plan `basic_trial`; pruebas en `tests/Feature/Auth/RegistrationTest.php` | Definir expiracion efectiva del trial, upgrade de plan y bloqueo/avisos al vencer. |
