...
| Contactos | MVP | CRUD, importacion, deduplicacion basica, fusion controlada y detalle operativo compacto funcionan dentro del alcance de empresa/canal. |
| Consentimientos | MVP | Otorgar/revocar funciona; falta historial mas completo y reglas de expiracion si aplican. |
| WhatsApp Cloud API | Operativo | Entrantes, salientes, estados reales, firma de webhook, ventana 24h y configuracion comercial compacta validados. |
| Conversaciones | Operativo | Inbox redisenado, no leidos, filtros rapidos, vista tipo chat, panel de detalles colapsable y composer inteligente para atencion diaria. |
...
| WA-COM-001 | WhatsApp comercial | Pantalla comercial de canales WhatsApp | MVP | P1 | Ruta `/integrations/whatsapp`, menu comercial `WhatsApp`, resumen operativo, listado compacto por canal, credenciales enmascaradas y pruebas en `tests/Feature/Auth/RegistrationTest.php` | Validar UX con usuarios comerciales y capturas por breakpoint. |
...
| WA-COM-003 | WhatsApp comercial | Crear/editar canal por empresa/sede | MVP | P1 | Rutas comerciales `POST/PATCH /integrations/whatsapp/channels`, formularios de crear/editar en modal, aislamiento por empresa/sede, limite de plan y pruebas en `tests/Feature/Auth/RegistrationTest.php` | Validar UX con varias sedes y conexion Meta real. |
| WA-COM-004 | WhatsApp comercial | Formulario seguro de credenciales Meta | MVP | P1 | Modal captura IDs Meta, secretos, URL Graph API y metadata de rotacion; conserva secretos vacios y audita sin exponer valores completos | Mejorar ayudas contextuales si soporte detecta dudas recurrentes. |
...
| WA-COM-008 | WhatsApp comercial | Guia en pantalla y documentacion operativa | MVP | P2 | Ayuda Meta desplegable en `/integrations/whatsapp` explica datos necesarios y orden de validacion sin exponer secretos; documentado en `docs/ux-ajustes-operativos.md` | Mantener ayuda compacta al ampliar integraciones. |
...
| CONV-001 | Conversaciones | Listado de conversaciones | Operativo | P0 | Inbox redisenado como panel de chats con busqueda por nombre/telefono, filtros rapidos, contexto de canal/asignacion, chat activo destacado, no leidos, auto-refresh, sonido opcional de nuevos mensajes y pruebas automatizadas | Validar con operadores en produccion y ajustar densidad si aumenta el volumen. |
| CONV-002 | Conversaciones | Timeline entrante/saliente | Operativo | P0 | Vista tipo chat integrada en `/conversations?conversation=...` con lista lateral, cabecera limpia, burbujas contenidas, fechas, estados, errores, panel de detalles colapsable y compositor inferior segun ventana 24h | Agregar scroll automatico al ultimo mensaje si el volumen lo exige. |
...
| CONV-007 | Conversaciones | Redisenio UX/UI tipo WhatsApp | Operativo | P1 | `/conversations` queda organizado en inbox, conversacion activa, panel de detalles colapsable y composer inteligente; documentado en `docs/conversaciones-flujo-operativo.md` y `docs/ux-ajustes-operativos.md`; validado con `NoiaChatMvpTest` y `npm run build` | Hacer validacion visual con operadores reales y capturas por breakpoint cuando se cierre QA. |
...
| UI-002 | Interfaz | Navegacion fija en layout operativo | Operativo | P1 | Sidebar fijo, header sticky y scroll limitado al contenido principal en `x-layouts.noia`; documentado en `docs/ux-ajustes-operativos.md`; validado con `NoiaChatMvpTest` y `npm run build` | Revisar pantallas largas adicionales y ajustar breakpoints si aparece contenido muy ancho. |
| UI-003 | Interfaz | Notificaciones transitorias con SweetAlert2 | Operativo | P1 | Mensajes `status` usan toast compacto, errores transitorios usan modal SweetAlert2 legible, auth/perfil/verificacion ya no muestran banners de sesion; documentado en `docs/ux-ajustes-operativos.md`; validado con `NoiaChatMvpTest` y `npm run build` | Mantener alertas persistentes inline solo para validaciones, plan o bloqueos que requieran contexto visible. |
...
| 2026-06-22 | CONV-007 implementado: redisenio UX/UI de `/conversations` con inbox, chat activo, historial con eventos de sistema, panel de detalles y composer inteligente segun ventana 24h. | Equipo NoiaChat |
